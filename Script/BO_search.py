...
def searchReport(URL,x_sap_logontoken,workbook):
    ...
    parentId = []
    query = {
        "query": "SELECT SI_DESCRIPTION,SI_ID,SI_NAME,SI_OWNER,SI_CREATION_TIME,SI_PARENT_FOLDER_CUID,SI_UPDATE_TS FROM CI_INFOOBJECTS WHERE SI_KIND IN ('Webi')"
        }
...
def searchUnx(URL,x_sap_logontoken,workbook):
    headers = set_headers_search(x_sap_logontoken)
    parentId = []
    query = {
    ...
    if r.ok:
        row = 0
        worksheetUniverse = workbook.add_worksheet("Unx")
        worksheetReport = workbook.add_worksheet("Unx-Report")
        worksheetFolder = workbook.add_worksheet("Unx Folder")
        print('OK Universe')
        response=r.json()
        entries = response['entries']
        for i in range(len(entries)):
            ...
            worksheetUniverse.write(i, col+2, description)
            worksheetUniverse.write(i, col+3, folder)
            slDocument = entries[i]['SI_SL_DOCUMENTS']
            totalReport = slDocument['SI_TOTAL']
            for k in range(totalReport):
                worksheetReport.write(row, 0, id)
                worksheetReport.write(row, 1, slDocument[str(k+1)])
                row += 1
            parentId.append(folder)
        parentId = list(dict.fromkeys(parentId))
...
def main():
    ...
    searchUnx(URL,x_sap_logontoken,workbook)
